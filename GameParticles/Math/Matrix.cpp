...
void Matrix::get(const Vect4D& row, Vect4D* t)
{
...
void Matrix::set(Vect4D* row, const Vect4D& t)
{
	// initialize the rows of the matrix
	 row->x = t.x;
	 row->y = t.y;
...
{ // matrix multiplications
	...
	__m128 tmp = _mm_set1_ps(this->v0.x);//v0
	__m128 tmp2 = _mm_mul_ps(tmp, mb.v0._m);//v0

	tmp = _mm_set1_ps(this->v0.y);//v0
	__m128 tmp3 = _mm_mul_ps(tmp, mb.v1._m);//v0

	tmp = _mm_set1_ps(this->v0.z);//v0
	__m128 tmp4 = _mm_mul_ps(tmp, mb.v2._m);//v0

	tmp = _mm_set1_ps(this->v0.w);//v0
	__m128 tmp5 = _mm_mul_ps(tmp, mb.v3._m);//v0

	tmp = _mm_add_ps(tmp2, tmp3);//v0
	tmp2 = _mm_add_ps(tmp, tmp4);//v0
	tmp3 = _mm_add_ps(tmp2, tmp5);//v0

	_mm_store_ps(&mOutput.m0, tmp3);//v0
	//------------------------------------
	tmp = _mm_set1_ps(this->v1.x);//v1
	tmp2 = _mm_mul_ps(tmp, mb.v0._m);//v1

	tmp = _mm_set1_ps(this->v1.y);//v1
	tmp3 = _mm_mul_ps(tmp, mb.v1._m);//v1

	tmp = _mm_set1_ps(this->v1.z);//v1
	tmp4 = _mm_mul_ps(tmp, mb.v2._m);//v1

	tmp = _mm_set1_ps(this->v1.w);//v1
	tmp5 = _mm_mul_ps(tmp, mb.v3._m);//v1

	tmp = _mm_add_ps(tmp2, tmp3);//v1
	tmp2 = _mm_add_ps(tmp, tmp4);//v1
	tmp3 = _mm_add_ps(tmp2, tmp5);//v1

	_mm_store_ps(&mOutput.m4, tmp3);//v1
	//------------------------------------
	tmp = _mm_set1_ps(this->v2.x);//v2
	tmp2 = _mm_mul_ps(tmp, mb.v0._m);//v2

	tmp = _mm_set1_ps(this->v2.y);//v2
	tmp3 = _mm_mul_ps(tmp, mb.v1._m);//v2

	tmp = _mm_set1_ps(this->v2.z);//v2
	tmp4 = _mm_mul_ps(tmp, mb.v2._m);//v2

	tmp = _mm_set1_ps(this->v2.w);//v2
	tmp5 = _mm_mul_ps(tmp, mb.v3._m);//v2

	tmp = _mm_add_ps(tmp2, tmp3);//v2
	tmp2 = _mm_add_ps(tmp, tmp4);//v2
	tmp3 = _mm_add_ps(tmp2, tmp5);//v2

	_mm_store_ps(&mOutput.m8, tmp3);//v2
	//------------------------------------
	tmp = _mm_set1_ps(this->v3.x);//v3
	tmp2 = _mm_mul_ps(tmp, mb.v0._m);//v3

	tmp = _mm_set1_ps(this->v3.y);//v3
	tmp3 = _mm_mul_ps(tmp, mb.v1._m);//v3

	tmp = _mm_set1_ps(this->v3.z);//v3
	tmp4 = _mm_mul_ps(tmp, mb.v2._m);//v3

	tmp = _mm_set1_ps(this->v3.w);//v3
	tmp5 = _mm_mul_ps(tmp, mb.v3._m);//v3

	tmp = _mm_add_ps(tmp2, tmp3);//v3
	tmp2 = _mm_add_ps(tmp, tmp4);//v3
	tmp3 = _mm_add_ps(tmp2, tmp5);//v3

	_mm_store_ps(&mOutput.m12, tmp3);//v3
...
Matrix& Matrix::operator/=(const float& rhs)
{
	...
	// using inverse multiply trick, faster than individual divides
...
Matrix& const Matrix::operator*=(const Matrix& mb)
{
	__m128 tmp = _mm_set1_ps(this->v0.x);//v0
	__m128 tmp2 = _mm_mul_ps(tmp, mb.v0._m);//v0
